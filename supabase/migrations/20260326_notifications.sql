-- Client notifications and email preferences
-- Notifications are created when relevant events occur; clients can view them and control email delivery.

-- Notification types: work_order_update, quote_sent, invoice_sent, task_update, new_message, contract_ready
...
-- Email preferences per user (clients only; admins don't need these)
...
-- Ensure default preferences exist for clients (call from app when loading settings)
-- Triggers to create notifications on key events
...
-- New message (when admin sends to client)
...
begin
  ...
  -- Only notify when admin sends (client is the recipient)
...
-- Task update (client-visible tasks on boards with client_id)
...
-- Also notify when task becomes client_visible (insert or update)
...
-- Contract ready for client (admin uploads contract)
